...
Some tip when starting to develop with Gtk4:

- the ui format has changed ...
- for the first project i just manually, used a definition from glade3 and, remove unwanted properties, and it was fine
- for the second project i did the same and got stuck with a entry/password field that was not focusable ... 
- after hours i used <pre>gtk4-builder-tool simplify --3to4 ...ui</pre> and everything was fine
- the import accepts the legacy True/False values for boolean properties, but they do not work as expected, using the gtk4-build-tool fixed this
